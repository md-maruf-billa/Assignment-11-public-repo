...
const ServiceToDo = () => {
    ...
    useEffect(() => {
        axios.get(`http://localhost:7000/booking-services?serviceProviderEmail=${currentUser.email}`)
            .then(data => setBookedServices(data.data))
    }, [])

    const updateStatus = (statusInfo, id) => {
        if (statusInfo == 'accept') {
            axios.put(`http://localhost:7000/update-booking/${id}`, { currentStatus: "working" })
                .then(data => {
                    Swal.fire({
                        title: "Congratulation",
                        text: "Updated status now working!",
                        icon: "success"
                    });
                    setLoading(true)
                })
        ...
        if (statusInfo == 'reject') {
            axios.put(`http://localhost:7000/update-booking/${id}`, { currentStatus: "reject" })
                .then(data => {
                    Swal.fire({
                        title: "Congratulation",
                        text: "Updated status now working!",
    ...
    return (
        <div className='min-h-[calc(100vh-112px)] mt-28 container mx-auto'>
            <PageTitle pgTitle={"Service To Do"} />
            {
...

                                                    {
                                                        bookedServices.map(booked =>
                                                            <tr key={booked._id}>
                                                                ...
                                                                <td class="px-4 py-4 text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap">{booked.customerEmail}</td>
                                                                ...
                                                                <td class="px-4 py-4 text-sm whitespace-nowrap">
                                                                    <div class="flex items-center gap-x-6">
                                                                        <button
                                                                            onClick={() => updateStatus('reject', booked._id)}
                                                                            class={`${booked.status=="reject" || booked.status=='completed'?'text-gray-300 btn-disabled':"hover:text-red-500 text-red-700"}  transition-colors duration-200 dark:hover:text-red-500  focus:outline-none`}>
                                                                        ...
                                                                        <button 
                                                                        className={`${booked.status == 'working' || booked.status=='completed'? 'btn-disabled text-gray-300':" text-green-500"} text-xl`}
                                                                        onClick={() => updateStatus('accept', booked._id)}>
                                                                            <FaCheck className='' />
                                                                        </button>
                                                                    </div>
                                                                </td>
