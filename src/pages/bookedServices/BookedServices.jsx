...
import { FaCalendar, FaEye, FaLocationDot } from 'react-icons/fa6';
...
const BookedServices = () => {
    ...
    useEffect(() => {
        axios.get(`http://localhost:7000/booking-services?customerEmail=${currentUser.email}`)
    ...
    const handelBookingDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`http://localhost:7000/delete-booking?_id=${id}`)
                    .then(data => {
                                                            ...
                                                            <tr key={booked._id}>
                                                                ...
                                                                <td class="px-12 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">
                                                                    <div class="inline-flex items-center px-3 py-1 rounded-full gap-x-2 bg-emerald-100/60 dark:bg-gray-800">
                                                                        <span class={`h-1.5 w-1.5 rounded-full ${booked?.status == 'pending' ? "bg-yellow-600" : booked.status == 'completed' ? "bg-emerald-500" : "bg-red-600"}`}></span>

                                                                        <h2 class={`text-sm font-normal ${booked?.status == 'pending' ? "text-yellow-600" : booked.status == 'completed' ? "text-emerald-500" : "text-red-600"}`}>{booked.status}</h2>
                                                                    </div>
                                                                </td>
